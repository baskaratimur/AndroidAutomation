...
## 🤖 AI / MCP Automation Workflow (Irit Token)

Jika Anda ingin membuat *Page Object Model* (POM) atau *Test Flow* baru menggunakan AI/LLM (seperti ChatGPT, Claude, atau Gemini) tanpa menghabiskan terlalu banyak batas token, Anda bisa menerapkan *workflow* berikut:

### 1. Eksekusi Script `clean_elemental`
Buka halaman di aplikasi yang ingin Anda ambil elemennya (pastikan HP/Emulator sedang nyala dan terhubung), kemudian cukup jalankan 1 perintah ini di terminal:

```bash
npx tsx scripts/clean_elemental.ts "nama_halaman"
# Contoh: npx tsx scripts/clean_elemental.ts "beranda"
```

Script tersebut otomatis akan melakukan 3 hal sekaligus:
- `adb shell uiautomator dump` (Menangkap layar device)
- `adb pull` (Menarik file ke PC Anda dan menyimpannya di `mcp/view_beranda.xml`)
- Menyaring XML (Membuang puluhan KB tag kosong dan menyisakan elemen yang relevan saja)

### 2. Berikan Hasilnya ke AI
*Script* di atas akan mencetak rangkuman elemen (yang super singkat) langsung di terminal Anda.
- **Salin (Copy)** hasil teks yang diprint di terminal tersebut.
- **Tempel (Paste)** ke chat AI dengan instruksi seperti: 
  > *"Buatkan file Page Object (POM) WebDriverIO (TypeScript) untuk halaman ini. Berikut struktur UI-nya: [paste_hasil_script_di_sini]"*
