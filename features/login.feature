...
Feature: Login Functionality
  ...
  Scenario: User failed to login with wrong password
    ...
    When user enters username "baskaratesting3" and password "WrongPassword123"
    And user checks the "Remember Me" checkbox
    And user clicks login button
    Then user should see the error message "Nama pengguna atau Kata sandi Anda belum benar. Coba lagi!"
  ...
  Scenario: User successfully login with valid data
    Given user is on the login page
    When user enters username "baskaratesting3" and password "Jejakin2023!"
    And user checks the "Remember Me" checkbox
    And user clicks login button
